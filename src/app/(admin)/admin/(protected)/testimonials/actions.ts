...
export async function updateTestimonial(id: string, input: TestimonialFormInput): Promise<void> {
  ...
  await adminDb().collection("testimonials").doc(id).update({ ...stripUndefined(input), updatedAt: serverTimestamp() });
  revalidateStorefront();
...
export async function setTestimonialActive(id: string, isActive: boolean): Promise<void> {
  await requireAdmin();
  await adminDb().collection("testimonials").doc(id).update({ isActive, updatedAt: serverTimestamp() });
  revalidateStorefront();
}
...
export async function reorderTestimonials(orderedIds: string[]): Promise<void> {
  ...
  orderedIds.forEach((id, index) => {
    batch.update(adminDb().collection("testimonials").doc(id), { order: index, updatedAt: serverTimestamp() });
  });
  await batch.commit();
  revalidateStorefront();
}
