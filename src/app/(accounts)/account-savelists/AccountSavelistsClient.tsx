...
export interface AccountSavelistsClientProps {
  productCardSettings?: ProductCardThemeConfig;
}

const AccountSavelistsClient = ({ productCardSettings }: AccountSavelistsClientProps) => {
    ...
    <div className="space-y-10 sm:space-y-12">
      ...
      {products.length > 0 && (
        <div className="grid grid-cols-1 gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3 ">
          {products.map((item) => (
            <ThemeProductCardAdapter key={item.id} data={item} productCardSettings={productCardSettings} />
          ))}
        </div>
      )}
    </div>
...
export default AccountSavelistsClient;
