 ...
 *
 * All of the writes below target different collections/documents with no data
 * dependency between them, so they run via one Promise.all instead of one after
 * another - each write pays its own Firestore round-trip latency (observed
 * 300-1800ms per step in production), and doing 9 of them sequentially was the
 * majority of the time this function's caller (createStore()'s background job)
 * was spending, risking the platform's function timeout even after the slow
 * synchronous work was moved out of the request's critical path.
 */
export async function installDefaultTheme(
  ...
  await Promise.all([
    ...
    storeDocRef
      ...
      .then(() => stage("ANNOUNCEMENT_BAR_WRITTEN")),
    ...
    ).then(() => stage("TESTIMONIALS_WRITTEN")),

    Promise.all(
      faqs.map((f) => faqsCol.doc().set({ ...f, createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() }))
    ).then(() => stage("FAQS_WRITTEN")),
